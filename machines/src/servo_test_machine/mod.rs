...
impl<T: ServoDevice + Default + std::fmt::Debug + 'static> Machine for ServoTestMachine<T> {
...
impl<T: ServoDevice> ServoTestMachine<T> {
    pub async fn emit_state(&mut self) {
        ...
        let position = servo.servo().get_position_actual().unwrap_or(0) as f32;
        let setpoint_position = 0.0; // ServoDevice trait doesn't expose setpoint
        let velocity = servo.servo().get_velocity_actual().unwrap_or(0) as f32;
        let error_code = servo.servo().get_error_code().unwrap_or(0);
        let lag_error = 0.0; // Lag error not available in current ServoDevice trait
