...
impl Guard for SystemSafeGuard {
    ...
    fn name(&self) -> &str {
        ...
        if !is_safe {
            tracing::warn!(
                "[StateMachine Hardware] Safety check FAILED: emergency_stop_ok={}, door_closed={}", 
                emergency_stop_ok, 
                door_closed
            );
        } else {
            tracing::info!("[StateMachine Hardware] Safety check PASSED");
        }
...
pub fn register_actions(registry: &mut ActionRegistry) {
    // Example actions - expand as needed
    registry.register(Arc::new(LogHardwareAction::new("Hardware action executed")));
    
    // You can register more actions here based on your hardware needs
    // registry.register(Arc::new(SetDigitalOutputAction::new("motor1".to_string(), true)));
}

/// Register all hardware guards into the registry
pub fn register_guards(registry: &mut GuardRegistry) {
    // Example guards - expand as needed
    registry.register(Arc::new(SystemSafeGuard));
    
    // You can register more guards here
    // registry.register(Arc::new(VariableTrueGuard::new("ready".to_string())));
}
